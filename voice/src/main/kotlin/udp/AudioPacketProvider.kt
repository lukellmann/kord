...
import dev.kord.voice.EncryptionMode.*
import dev.kord.voice.XSalsa20_CONSTRUCTOR_DEPRECATION_CYCLE
import dev.kord.voice.XSalsa20_DEPRECATION
import dev.kord.voice.XSalsa20_PROPERTY_DEPRECATION_CYCLE
...
public abstract class AudioPacketProvider private constructor(public val key: ByteArray, internal val mode: Mode) {
    ...
    @Deprecated(
        "$XSalsa20_DEPRECATION An 'AudioPacketProvider' instance can be created without a 'nonceStrategy' in which " +
            "case this property will throw an 'UnsupportedOperationException'. $XSalsa20_PROPERTY_DEPRECATION_CYCLE",
        level = DeprecationLevel.WARNING,
    )
    public val nonceStrategy: @Suppress("DEPRECATION") NonceStrategy
        get() = when (mode) {
            is Mode.Legacy -> mode.nonceStrategy
            is Mode.New -> throw UnsupportedOperationException(
                "This AudioPacketProvider instance was created without a nonceStrategy."
            )
        }

    @Deprecated(
        "$XSalsa20_DEPRECATION Construct an 'AudioPacketProvider' instance with an 'EncryptionMode' instead of a " +
            "'nonceStrategy'. $XSalsa20_CONSTRUCTOR_DEPRECATION_CYCLE",
        ReplaceWith(
            "AudioPacketProvider(key, EncryptionMode.AeadXChaCha20Poly1305RtpSize)",
            imports = ["dev.kord.voice.udp.AudioPacketProvider", "dev.kord.voice.EncryptionMode"],
        ),
        DeprecationLevel.WARNING,
    )
    public constructor(key: ByteArray, nonceStrategy: @Suppress("DEPRECATION") NonceStrategy) :
        this(key, Mode.Legacy(nonceStrategy))
...
public class DefaultAudioPacketProvider : AudioPacketProvider {
    @Deprecated(
        "$XSalsa20_DEPRECATION Construct a 'DefaultAudioPacketProvider' instance with an 'EncryptionMode' instead of " +
            "a 'nonceStrategy'. $XSalsa20_CONSTRUCTOR_DEPRECATION_CYCLE",
        ReplaceWith(
    ...
    private val delegate = when (mode) {
        ...
        is Mode.New -> @Suppress("DEPRECATION") when (val encryptionMode = mode.encryptionMode) {
            AeadAes256GcmRtpSize -> EncryptedPacketCreatorProviderDelegate(AeadAes256GcmRtpSizeVoicePacketCreator(key))
            ...
            is Unknown -> throw UnsupportedOperationException("Unknown encryption mode $encryptionMode")
        }
    }
